...
export default class ChoosePlan extends React.Component {
    ...
    render() {
        ...
        return this.state.plans.length > 0 ? <Container id="pricing" className="px-0 mt-6"><H1 className="text-center" title="Vous avez déja un abonnement Asking Franklin Premium actif."/></Container> : <Container id="pricing" className="px-0 mt-6">
        ...
        <Row className={ this.state.selectedPlan === 0 ?
                classListRow
            : this.state.selectedPlan === 1 ?
                'plan-preselected plan-preselected-monthly' + classListRow
            : this.state.selectedPlan === 2 &&
                'plan-preselected plan-preselected-annual' + classListRow
        }>
